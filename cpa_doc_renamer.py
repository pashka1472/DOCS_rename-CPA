...
def find_payer_or_issuer(text: str) -> str | None:
    return first_match(
        [
            ...
            r"Employer(?:'s)? name(?: and address)?\s*[:\-]?\s*([^\n]+)",
            r"Issuer(?:'s)? name\s*[:\-]?\s*([^\n]+)",
            r"Lender(?:'s)? name\s*[:\-]?\s*([^\n]+)",
        ],
        text,
...
def find_account_last4(text: str) -> str | None:
    patterns = [
        r"account(?: number| no\.?| #)?[^\n\d]*(?:x{2,}|\*{2,}|ending in|ends in|last four)?\D*(\d{4})(?!\d)",
        r"(?:x{2,}|\*{2,}|ending in|last four)\D*(\d{4})(?!\d)",
    ]
...
def classify_document(text: str) -> NamingResult:
    ...
    if "form 1098" in lowered or re.search(r"\b1098\b", lowered):
        lender = broker or issuer or "unknown_lender"
        return NamingResult(f"1098_{slug_company(lender)}", "1098", lender)

    if "schedule k-1" in lowered or re.search(r"\bk-?1\b", lowered):
    ...
    if "nonemployee compensation" in lowered or re.search(r"\b1099\s*-?\s*nec\b", lowered):
        name = issuer or "unknown_payer"
        return NamingResult(f"1099_NEC_{sanitize_filename_part(name)}", "1099-NEC", name)

    if "miscellaneous" in lowered or re.search(r"\b1099\s*-?\s*misc\b", lowered):
        name = issuer or "unknown_payer"
    ...
    if "dividend" in lowered or re.search(r"\b1099\s*-?\s*div\b", lowered):
        return NamingResult(f"1099_dividends_{broker_slug}_{acct}", "1099-DIV", broker, account_last4)
    if "interest income" in lowered or re.search(r"\b1099\s*-?\s*int\b", lowered):
