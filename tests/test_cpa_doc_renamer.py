...
def test_sample_1098_lender_colon():
    text = "1098_filled_sample\n\nLender: Bank of America\nBorrower: John A Smith"
    result = classify_document(text)
    assert result.filename_stem == "1098_bank_of_america"


def test_sample_consolidated_1099_broker_colon_without_account():
    text = "Consolidated_1099_sample\n\nBroker: Fidelity Investments\nInterest $245.32\nOrdinary dividends"
    result = classify_document(text)
    assert result.filename_stem == "1099_consolidated_fidelity_unknown_account"


def test_sample_k1_partnership_colon():
    text = "K1_filled_sample\n\nPartnership: ABC Holdings LLC\nPartner: John A Smith"
    result = classify_document(text)
    assert result.filename_stem == "K1_ABC Holdings LLC"


def test_sample_1099_div_underscore_title_and_payer_colon():
    text = "1099_DIV_filled_sample\n\nPayer: Fidelity Investments LLC\nTIN: XXX-XX-4821"
    result = classify_document(text)
    assert result.filename_stem == "1099_dividends_fidelity_4821"


def test_sample_1099_misc_underscore_title_and_payer_colon():
    text = "1099_MISC_filled_sample\n\nPayer: Sunrise Properties LLC\nTIN: XXX-XX-4821"
    result = classify_document(text)
    assert result.filename_stem == "1099_misc_Sunrise Properties LLC"


def test_sample_1099_nec_underscore_title_and_payer_colon():
    text = "1099_NEC_filled_sample\n\nPayer: ABC Construction LLC\nNonemployee compensation: $42,680.00"
    result = classify_document(text)
    assert result.filename_stem == "1099_NEC_ABC Construction LLC"


def test_sample_1099_int_account_uses_last_four_digits():
    text = "Form 1099-INT Interest Income\nGarden State Community Bank\nAccount number SAV-77120493"
    result = classify_document(text)
    assert result.filename_stem == "1099_int_garden_state_community_bank_0493"


def test_sample_w2_multiline_employer_name():
    text = "Form W-2 Wage and Tax Statement\nEmployer's name, address, and ZIP code\nHudson Sample Manufacturing Inc."
    result = classify_document(text)
    assert result.filename_stem == "W2_Hudson Sample Manufacturing Inc"
